...
const HEADER_MAP: { field: keyof TableEventRow; words: string[] }[] = [
  { field: 'dateText', words: ['date', 'datum', 'fecha', 'data', 'dato', 'datas', 'quand', 'when', 'termin', 'tanggal', 'tarih', 'pvm', 'paivamaara', '日付', '開催日'] },
  { field: 'name', words: ['name', 'event', 'race', 'swim', 'nombre', 'evento', 'prueba', 'nom', 'epreuve', 'veranstaltung', 'wettkampf', 'gara', 'evenement', 'wedstrijd', 'arrangement', 'lopp', 'tavling', 'zawody', 'nazwa', 'yarisma', 'kilpailu', 'nimi', '大会', '大会名'] },
  { field: 'locationText', words: ['location', 'place', 'venue', 'where', 'lugar', 'ubicacion', 'sede', 'lieu', 'ort', 'luogo', 'plaats', 'sted', 'plats', 'miejsce', 'yer', 'paikka', 'local', '会場', '開催地'] },
  { field: 'distanceText', words: ['distance', 'distancia', 'distanza', 'afstand', 'entfernung', 'strecke', 'dystans', 'mesafe', 'matka', 'lange', 'length', 'km', '距離'] },
  { field: 'timeText', words: ['time', 'start', 'hora', 'heure', 'zeit', 'ora', 'tijd', 'tid', 'godzina', 'saat', 'aika', '時間'] },
];
...
function coordsFromRow(html: string): { lat: number | null; lon: number | null } {
  const brace = /\{\s*(-?\d{1,3}\.\d{3,}),\s*(-?\d{1,3}\.\d{3,})\s*\}/.exec(html);
  if (brace?.[1] && brace[2]) return { lat: Number(brace[1]), lon: Number(brace[2]) };

  const q = /[?&](?:q|mlat)=(-?\d{1,3}\.\d{3,})[,&]/.exec(html);
  const qLon = /[?&](?:q=-?\d{1,3}\.\d{3,},|mlon=)(-?\d{1,3}\.\d{3,})/.exec(html);
  if (q?.[1] && qLon?.[1]) return { lat: Number(q[1]), lon: Number(qLon[1]) };

  const generic = /(-?\d{1,3}\.\d{4,}),\s?(-?\d{1,3}\.\d{4,})/.exec(html);
  if (generic?.[1] && generic[2]) return { lat: Number(generic[1]), lon: Number(generic[2]) };
...
export function pageYearFrom(html: string, now = new Date()): number | null {
  ...
  const sources = [$('title').text(), $('h1').first().text(), $('h2').first().text()];
...
export function extractTableEvents(html: string, now = new Date()): TableExtractionResult {
  ...
  if (allRows.length < 3) return { rows: [], pageYear, warnings: ['Largest table has too few rows to be a calendar'] };

  // Header row: the first row with <th>, else the first row whose cells
  // read like headers rather than data.
  const headerRow = allRows.find((r) => $(r).find('th').length > 0) ?? allRows[0]!;
  const headerCells = $(headerRow).find('th,td').toArray().map((c) => fold(cellText($, c)));

  const columnField: (keyof TableEventRow | null)[] = headerCells.map((h) => {
    for (const { field, words } of HEADER_MAP) {
      if (words.some((w) => h === fold(w) || h.includes(fold(w)))) return field;
    }
    return null;
  });

  if (!columnField.includes('dateText') && !columnField.includes('name')) {
    return { rows: [], pageYear, warnings: [`No date or name column recognised in headers: ${headerCells.join(' | ')}`] };
  }
  ...
  for (const tr of allRows) {
    ...
    const row: TableEventRow = {
      dateText: null, name: null, locationText: null, distanceText: null,
    ...
    const href = scope
      ...
      .toArray()
      .map((a) => $(a).attr('href') || '')
      .find((h) => h && !/^#/.test(h) && !/(google\.[a-z.]+\/maps|bing\.com\/maps|openstreetmap\.org)/i.test(h));
    row.url = href || null;
  ...
  if (rows.length === 0) warnings.push('Table matched headers but produced no usable event rows');
