...
export const AI_SYSTEM_PROMPT = [
  ...
  "5. A WAVE is not an event. Age groups, categories, heats, start groups, team classifications and numbered starts within a single race all belong to that ONE race — 'Boys 13 and Under', 'Disabled and 71yr/over', 'Company Teams', 'Event 1', 'Wave 3' are not events and must never be rows of their own. If a page breaks one race into start groups, return the RACE, once, with its own name.",
  "5b. An ENTRY OPTION is not an event either. A festival page that offers 'Sprint', 'Olympic', 'Middle', 'Half Marathon', '10K', '1 Mile', 'Starter' or '9-10 years' is listing the ways to enter ONE event, not naming several. These are exactly rule 4's distances and rule 5's categories wearing the page's own words: return the festival, once, under the name the page gives it, with those distances in distanceText. If the only name you can find for a row is a distance, a format or an age range, it is not a row.",
  "6. Never return a placeholder as a name. 'Event 1', 'Race 2', 'Entry Information' and section headings are not what the race is called. If you cannot find the race's actual name, return an empty list rather than a numbered stand-in.",
  '7. Where the page prints a field LABEL before a value ("Lieu :", "Ville :", "Organisé par :", "Location:", "Datum:"), copy the value only, not the label. "Lieu : ANNECY Ville : ANNECY (74000)" is a location of "ANNECY (74000)". This is still copying, not interpreting — take the answer, leave the question.',
  '8. Do not judge whether an event qualifies as open-water swimming, and do not filter on that basis. Include pool galas, triathlons and past events if the page lists them; a separate classifier decides what to keep.',
  '9. If the page is not an event listing at all — a homepage, an article, a results archive with no forthcoming dates — return an empty list. An empty list is a correct and useful answer.',
  '',
